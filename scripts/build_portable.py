...
def write_launcher_and_readme() -> None:
    ...
    (BUILD / "使用说明.txt").write_text(
        "MAgent —— CUMCM 数模全流程 Agent 软件（绿色版）\n"
        "=================================================\n\n"
        "使用步骤：\n"
        "1. 双击「启动MAgent.bat」→ 弹出 MAgent 软件窗口（这是正常形态，\n"
        "   不是浏览器；关闭窗口即退出软件）\n"
        "2. 首次使用：点左侧「⚙ 设置」→ 添加模型服务，填入你自己的模型 API Key\n"
        "   （默认 DeepSeek：https://api.deepseek.com / deepseek-chat，\n"
        "    任何 OpenAI 兼容服务改 Base URL 和模型名即可）→ 点「测试连接」\n"
        "   同一页的「阶段路由」可给不同阶段指定不同模型\n"
        "3. 新建项目：填项目路径、题目标题，选择题面文件（PDF/DOCX/MD/TXT）→ 创建\n"
        "   数据附件：点「选择数据文件夹…」可整个文件夹上传（Excel/CSV 等），\n"
        "   自动按原目录结构存入项目的 data/ 目录（也可直接多选文件）\n"
        "4. 在流水线卡片上点「启动」，从赛题分析开始逐阶段运行\n\n"
        "常见问题：\n"
        "- 桌面窗口打不开时，用「启动MAgent（浏览器模式）.bat」备用，\n"
        "  并把控制台里的报错发给作者；\n"
        "- 论文编译需要 TeX：未安装时论文阶段交付 .tex，doctor 会提示；\n"
